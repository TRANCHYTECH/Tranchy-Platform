...
internal static class QuestionMapper
{
    static QuestionMapper() =>
        TypeAdapterConfig<CreateQuestionRequest, Data.Question>
            ...
            .Map(dest => dest.CreatedByUserId,
                _ => MapContext.Current!.Parameters[nameof(Data.Question.CreatedByUserId)])
            .Map(dest => dest.QueryIndex, _ => MapContext.Current!.Parameters[nameof(Data.Question.QueryIndex)]);

    internal static Data.Question ToEntity(this CreateQuestionRequest request, string userId, long queryIndex) =>
