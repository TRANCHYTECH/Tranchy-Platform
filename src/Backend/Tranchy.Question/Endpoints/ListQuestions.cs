...
public class ListQuestions : IEndpoint
{
    ...
    {
        var acceptedStatuses = new[] { QuestionStatus.Accepted, QuestionStatus.InProgress };
        var questions = await DB.Find<Data.Question>().Match(q => acceptedStatuses.Contains(q.Status)).ExecuteAsync(cancellation);
        questions.ForEach(q => q.SetPermissions(tenant.UserId));
