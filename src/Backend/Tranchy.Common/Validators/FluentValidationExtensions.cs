...
{
    public static async Task TryValidate<T>(this IValidator<T> validator, T instance, CancellationToken cancellation = default)
    {
        var result = await validator.ValidateAsync(instance, cancellation);
        CheckResultThrowException(result);
    }

    public static async Task TryValidate<T>(this IValidator<T> validator, T instance, CancellationToken cancellation, params string[] properties)
    {
        var result = await validator.ValidateAsync(instance, opt => opt.IncludeProperties(properties), cancellation: cancellation);
        CheckResultThrowException(result);
    }
