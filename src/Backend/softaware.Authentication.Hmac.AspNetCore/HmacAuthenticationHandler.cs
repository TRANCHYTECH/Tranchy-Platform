...
namespace softaware.Authentication.Hmac.AspNetCore
{
    internal class HmacAuthenticationHandler : AuthenticationHandler<HmacAuthenticationSchemeOptions>
    {
        ...
        {
            if (Options.AuthorizationProvider is null)
            {
        ...
        private static (bool IsValidHeader, HmacAuthenticationHeaderValues Values) GetAuthenticationValues(string rawAuthenticationHeader)
        {
            ...
            softaware.Authentication.Hmac.Client.HmacHashingMethod hmacHashingMethod; // Default value in previous library versions before changing header to include hashing algorithm.
            if (hasHashingAlgorithmProperties)
            {
                if (Enum.TryParse<HmacHashingMethod>(authenticationHeaderArray[0], out var hhm))
                {
                    hmacHashingMethod = hhm;
                }
                else
                {
                    throw new NotSupportedException($"Hmac hashing method {authenticationHeaderArray[0]} is not supported.");
                }
            }
            else
            {
            ...
            softaware.Authentication.Hmac.Client.RequestBodyHashingMethod requestBodyHashingMethod; // Default value in previous library versions before changing header to include hashing algorithm.
            if (hasHashingAlgorithmProperties)
            {
                if (Enum.TryParse<RequestBodyHashingMethod>(authenticationHeaderArray[1], out var rhm))
                {
                    requestBodyHashingMethod = rhm;
                }
                else
                {
                    throw new NotSupportedException($"Request body hashing method {authenticationHeaderArray[1]} is not supported.");
                }
            }
            else
            {
        ...
        {
            if (Options.TrustProxy && req.Headers.TryGetValue("X-Forwarded-Proto", out var scheme))
            {
                return scheme;
            }
